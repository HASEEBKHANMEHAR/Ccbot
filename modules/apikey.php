    ...
    if($antispam != False){
      ...
      return;
        ...
        if(preg_match_all("/sk_(test|live)_[A-Za-z0-9]+/", $sk, $matches)) {
            ...
            curl_setopt($ch, CURLOPT_POSTFIELDS, "card[number]=5154620061414478&card[exp_month]=01&card[exp_year]=2026&card[cvc]=235");
            ...
            if(strpos($result, 'error') == False || (stripos($result, '"type": "card_error"'))){
                ...
                bot('editMessageText',[
                    'chat_id'=>$chat_id,
                    'message_id'=>$messageidtoedit,
                    'text'=>"<b>SK is live and Successfully Added Your API Key!✅
You can now check cards using /schk</b>",
              ...
              if(strpos($result, 'error') == False || (stripos($result, 'rate_limit'))){
                updateAPIKey($userId,$sk);
                file_put_contents('sk.txt', "\n$sk",FILE_APPEND);
                bot('editMessageText',[
                    'chat_id'=>$chat_id,
                    'message_id'=>$messageidtoedit,
                    'text'=>"<b>Sk is Rate limited but Successfully Added Your API Key!✅
You can now check cards using /schk</b>",
                    'parse_mode'=>'html',
                    'disable_web_page_preview'=>'true'
            ...
            elseif(stripos($result, 'error')){
                bot('editMessageText',[
                    'chat_id'=>$chat_id,
                    'message_id'=>$messageidtoedit,
                    'text'=>"<b>This SK Key is Dead! Provide$ me a Live SK Key $result </b>",
                    'parse_mode'=>'html',
                    'disable_web_page_preview'=>'true'
