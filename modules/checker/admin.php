...
////////////====[GENERATE CODE]====////////////
if (strpos($message, "/code ") === 0 && $userId == $config['adminID']) {
    $args = explode(" ", substr($message, 6));
    $duration = $args[0]; // e.g., "1d", "7d", etc.

    // Check if the provided duration is valid (You can add more validation as needed)
    if (!preg_match('/^\d+[dw]$/', $duration)) {
        bot('sendmessage', [
            'chat_id' => $chat_id,
            'text' => "Invalid code duration! Use format like '1d' or '7d'.",
            'parse_mode' => 'html',
            'reply_to_message_id' => $message_id,
        ]);
        return;
    ...
    $licenseKey = generateLicenseKey($duration);

    bot('sendmessage', [
        'chat_id' => $chat_id,
        'text' => "<b>Generated License Key:</b>\n$licenseKey",
        'parse_mode' => 'html',
        'reply_to_message_id' => $message_id,
    ]);
}
...
if(strpos($message, "/users ") === 0 and $userId == $config['adminID']){
  $uStats = fetchUser();
